...
Получается, что сигнатуру метода _arraycopy()_ с ключевым словом _native_ мы можем наблюдать в классе [System](System), а вот реализация, написанная на языках C/C++, находится совсем в другом месте. Она является частью скомпилированной в виде машинного (бинарного) кода библиотеки внутри JVM.  
...
Над сигнатурой метода мы также можем наблюдать аннотацию _@IntrinsicCandidate_ (до Java 16 она называлась _@HotSpotIntrinsicCandidate_), сообщающую о том, что JVM вместо вызова кода, написанного на другом языке.
