...
```java
(_list_of_parameters_) -> _expression_;
```
здесь
...
(double a, double b, double c ) -> {
  if (((a+b)<c) || ((a+c)<b) || ((b+c)<a))
    ...
    double s = Math._sqrt_(p*(p-a)*(p-b)*(p-c)); // площадь по формуле Герона
    return s;
  }
}
```
...
interface InterfaceName {
  _return_type_ MethodName(_list_of_parameters_);
}
```
здесь
...
```
#### Лямбда-выражение, использующее шаблон (обобщение) ####
