...
Метод addAll() принимает коллекцию в качестве параметра, поэтому вы можете передать либо список, либо Java Set в качестве параметра. Другими словами, вы можете добавить все элементы из списка или установить в список с помощью addAll() . 
...
```
Результатом выполнения этого примера будет: ==true==
...
Если список содержит объекты, реализующие интерфейс Comparable (java.lang.Comparable), тогда объекты могут сравнивать себя друг с другом. В этом случае вы можете отсортировать список следующим образом:
```java
List<String> list = new ArrayList<>();
...
Четвертый способ итерации списка Java - через Java Stream API. Чтобы выполнить итерацию списка Java, вы должны сначала получить поток из списка. Получение потока из списка в Java осуществляется путем вызова метода List stream(). Вот пример получения потока Java из списка Java:
```java
