...
Интерфейс Java Map не является подтипом интерфейса Collection. Поэтому он ведет себя немного иначе, чем остальные типы коллекций.
#### Реализации Java Map ####
...
Наиболее часто используемыми реализациями карт являются HashMap и TreeMap. Каждая из этих реализаций Map ведет себя немного по-разному в отношении порядка элементов при итерации карты и времени (big O notation), необходимого для вставки элементов в Map и доступа к ним.

HashMap отображает ключ и значение. Это не гарантирует какого-либо порядка элементов, хранящихся внутри карты. TreeMap также отображает ключ и значение. Кроме того, это гарантирует порядок, в котором повторяются ключи или значения, то есть порядок сортировки ключей или значений.

Реализация HashMap, как правило, является самой быстрой из двух реализаций Map, поэтому всякий раз, когда вам не нужно сортировать элементы на карте, вы можете просто использовать HashMap. В противном случае используйте TreeMap.
#### Создание Map ####
...
map.put("key", 123);
```
Значение, передаваемое методу put() в приведенном выше примере, является примитивным значением int. Однако Java автоматически помещает его в экземпляр Integer, поскольку метод put() требует экземпляр Oject как в качестве ключа, так и в качестве значения. Автоупаковка также произошла бы, если бы вы передали примитив в качестве ключа методу put().
...
Начиная с Java 8, вы можете использовать Java Stream для перебора ключей Java Map. Потоковый интерфейс является частью Java Stream API, который был добавлен в Java 8. Сначала вы получаете набор ключей с карты, и из этого вы можете получить поток. Вот пример итерации ключей Java Map через поток:
```java
Map<String, String> map = new HashMap<>();
...
Первый способ перебрать все значения, хранящиеся в Java Map, - это получить экземпляр value Iterator из набора значений и выполнить его итерацию. Вот как выполняется итерация значений, хранящихся в Java Map, с помощью итератора значений:
```java
Map<String, String> map = new HashMap<>();
...
```
Поскольку множество Set неупорядочено, у вас нет никаких гарантий относительно порядка, в котором повторяются значения в множестве значений. Однако, если вы используете TreeSet, вы все равно можете контролировать этот порядок.
...
Третий способ перебора значений, хранящихся в Java Map, - это использование потока значений, используя Java Stream API. Сначала вы получаете набор значений из карты, а из набора значений вы можете получить поток. Вот пример итерации значений Java Map через поток создания значений:
```java
Map<String, String> map = new HashMap<>();
...
Метод Map compute() принимает ключевой объект и лямбда-выражение в качестве параметров. Лямбда-выражение должно реализовывать java.util.function. Двухфункциональный интерфейс. Вот пример вызова метода Java Map compute():
```java
...
```
Этот пример на самом деле просто возвращает постоянное значение - строку 123 . Однако лямбда-выражение могло бы вычислить значение любым необходимым ему способом - например, извлечь значение из другого объекта или объединить его с другими значениями и т.д.
#####  computeIfPresent() #####

Метод Map computeIfPresent() работает противоположно методу computeIfAbsent(). Он вызывает лямбда-выражение, переданное ему в качестве параметра, только в том случае, если запись для этого ключа уже существует в карте. Вот пример вызова метода computeIfPresent():
```java
