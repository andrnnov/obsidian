...
Рисует `Image` в указанном месте на входе `Graphics`. Данное значение `Component` передается методу `Graphic`’s `drawImage()` как `ImageObserver` (напомним, что `java.awt.Component` реализует `ImageObserver`), если наблюдатель изображений не был явно задан.
